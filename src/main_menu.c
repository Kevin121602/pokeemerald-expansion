 ...
 * Task_NewGameBirchSpeech_StartBirchBUNEARYPlatformFade
 * Task_NewGameBirchSpeech_StartBirchBUNEARYPlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchBUNEARY
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForBUNEARY
 * Task_NewGameBirchSpeechSub_WaitForBUNEARY
...
static void Task_NewGameBirchSpeechSub_WaitForBUNEARY(u8);
static void Task_NewGameBirchSpeech_StartBirchBUNEARYPlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchBUNEARY(u8);
...
#define tBUNEARYSpriteId data[9]
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tBUNEARYSpriteId;
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, 0x0000FFFF, SPECIES_BUNEARY);
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForBUNEARY;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForBUNEARY(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tBUNEARYSpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(1);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchBUNEARYPlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchBUNEARYPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tBUNEARYSpriteId].invisible = TRUE;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tBUNEARYSpriteId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchBUNEARY;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchBUNEARY(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        spriteId = gTasks[taskId].tBUNEARYSpriteId;
        gSprites[spriteId].x = 100;
        gSprites[spriteId].y = 75;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gSprites[gTasks[taskId].tBUNEARYSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tBUNEARYSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static u8 NewGameBirchSpeech_CreateBUNEARYSprite(u8 x, u8 y)
{
    return CreateMonPicSprite_Affine(SPECIES_BUNEARY, SHINY_ODDS, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    BUNEARYSpriteId = NewGameBirchSpeech_CreateBUNEARYSprite(100, 0x4B);
    gSprites[BUNEARYSpriteId].callback = SpriteCB_Null;
    gSprites[BUNEARYSpriteId].oam.priority = 0;
    gSprites[BUNEARYSpriteId].invisible = TRUE;
    gTasks[taskId].tBUNEARYSpriteId = BUNEARYSpriteId;
