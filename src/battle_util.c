...
u32 CheckMoveLimitations(enum BattlerId battler, u8 unusableMoves, u16 check, bool8 illusion)
{
    enum Move move;
...
bool32 TryChangeBattleTerrain(enum BattlerId battler, u32 statusFlag, bool32 viaAbility)
{
...
enum HoldEffect GetBattlerHoldEffectIllusion(enum BattlerId battler)
{
    if (gBattleMons[battler].volatiles.embargo)
        return HOLD_EFFECT_NONE;
    if (gFieldStatuses & STATUS_FIELD_MAGIC_ROOM)
...
bool32 IsBattlerProtected(struct BattleContext *ctx)
{
    ...
    if (IsCraftyShieldProtected(ctx->battlerAtk, ctx->battlerDef, ctx->move))
        ...
        isProtected = TRUE;
    else if (gProtectStructs[ctx->battlerDef].protected == PROTECT_FLOWER_SHIELD && !IsBattleMoveStatus(ctx->move))
        isProtected = TRUE;
    else if (gProtectStructs[ctx->battlerDef].protected == PROTECT_DEFEND_ORDER && !IsBattleMoveStatus(ctx->move))
        isProtected = TRUE;
    else
        isProtected = FALSE;
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct BattleContext *ctx)
{
    ...
    case ABILITY_ANALYTIC:
        ...
        else if (IsLastMonToMove(battlerAtk) && moveEffect != EFFECT_FUTURE_SIGHT)
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        break;
...
static inline uq4_12_t GetScreensModifier(struct BattleContext *ctx)
{
    ...
    if (gMovesInfo[ctx->move].additionalEffects[0].moveEffect == MOVE_EFFECT_BREAK_SCREEN || ctx->isSelfInflicted)
    {
        return UQ_4_12(1.0);
    }
